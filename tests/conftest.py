...
def _plan_runner():
    ...
    def run_plan(fixture_path=None, targets=None, refresh=True, **tf_vars):
        ...
        with tempfile.TemporaryDirectory(
            prefix=fixture_prefix, dir=fixture_parent
        ) as tmp_path:
            ...
            tf = tftest.TerraformTest(
                tmp_path, BASEDIR, os.environ.get("TERRAFORM", "terraform")
            )
...
def e2e_plan_runner(_plan_runner):
    ...
    def run_plan(
        ...
        **tf_vars
        ...
        root_module = plan.root_module["child_modules"][0]
        modules = dict(
            (mod["address"], mod["resources"]) for mod in root_module["child_modules"]
        )
