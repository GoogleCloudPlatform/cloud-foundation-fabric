...
def test_iam(resources):
    ...
    bindings = [
        r["values"] for r in resources if r["type"] == "google_pubsub_topic_iam_binding"
    ]
...
def test_subscriptions(resources):
    ...
    subs = [r["values"] for r in resources if r["type"] == "google_pubsub_subscription"]
    assert len(subs) == 1
...
def test_subscription_iam(resources):
    ...
    bindings = [
        r["values"]
        for r in resources
        if r["type"] == "google_pubsub_subscription_iam_binding"
    ]
    assert len(bindings) == 1
