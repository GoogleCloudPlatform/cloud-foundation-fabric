...
def test_vpc_shared(plan_runner):
    ...
    _, resources = plan_runner(
        shared_vpc_host="true", shared_vpc_service_projects='["tf-a", "tf-b"]'
    )
...
def test_vpc_peering(plan_runner):
    ...
    peerings = [
        r["values"] for r in resources if r["type"] == "google_compute_network_peering"
    ]
    assert [p["name"] for p in peerings] == ["my-vpc-my-peer-vpc", "my-peer-vpc-my-vpc"]
...
def test_vpc_routes(plan_runner):
    ...
    for next_hop_type, next_hop in _VAR_ROUTES_NEXT_HOPS.items():
        ...
        assert len(resources) == 3
        resource = [
            r for r in resources if r["values"]["name"] == "my-vpc-next-hop-test"
        ][0]
