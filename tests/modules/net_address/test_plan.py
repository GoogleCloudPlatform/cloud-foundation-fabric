...
def test_external_addresses(plan_runner):
    ...
    assert set(r["values"]["address_type"] for r in resources) == set(["EXTERNAL"])
...
def test_internal_addresses(plan_runner):
    ...
    assert [r["values"]["name"] for r in resources] == ["one", "two"]
    assert set(r["values"]["address_type"] for r in resources) == set(["INTERNAL"])
    assert [r["values"]["region"] for r in resources] == [
        "europe-west1",
        "europe-west2",
    ]
...
def test_internal_addresses_config(plan_runner):
    ...
    _, resources = plan_runner(
        internal_addresses=addresses, internal_addresses_config=config
    )
    assert [r["values"]["name"] for r in resources] == ["one", "two"]
    assert set(r["values"]["address_type"] for r in resources) == set(["INTERNAL"])
...
def test_psa_config(plan_runner):
    psa_addresses = (
        '{cloudsql-mysql={address="10.199.0.0", network="foobar", prefix_length = 24}}'
    )
