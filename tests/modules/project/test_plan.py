...
def test_service_encryption_keys(plan_runner):
    ...
    _, resources = plan_runner(
        service_encryption_key_ids=('{compute=["key1"], storage=["key1", "key2"]}')
    )
    key_bindings = [
        r["index"] for r in resources if r["type"] == "google_kms_crypto_key_iam_member"
    ]
...
def test_service_encryption_key_dependencies(plan_runner):
    ...
    _, resources = plan_runner(
        service_encryption_key_ids=('{compute=["key1"], dataflow=["key1", "key2"]}')
    )
    key_bindings = [
        r["index"] for r in resources if r["type"] == "google_kms_crypto_key_iam_member"
    ]
