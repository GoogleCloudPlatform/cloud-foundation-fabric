...
output "regional_rules" {
  ...
  value       = try([for k in google_compute_region_network_firewall_policy_rule.default : k.id], "")
}
...
output "regional_network_association" {
  description = "Global association name"
  value       = try(google_compute_region_network_firewall_policy_association.default[0].id, "")
}
