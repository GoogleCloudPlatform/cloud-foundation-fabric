...
# tfdoc:file:description Global security policy and rules.
...
resource "google_compute_security_policy" "global" {
  ...
  dynamic "adaptive_protection_config" {
    for_each = var.adaptive_protection_config == null ? [] : [""]
    content {
      dynamic "layer_7_ddos_defense_config" {
        for_each = (
          var.adaptive_protection_config.layer_7_ddos_defense == null
          ? []
          : [var.adaptive_protection_config.layer_7_ddos_defense]
        )
        iterator = l7
        content {
          enable          = l7.value.enable
          rule_visibility = l7.value.rule_visibility
          dynamic "threshold_configs" {
            for_each = l7.value.threshold_configs
            iterator = tc
            content {
              name                                    = tc.key
              auto_deploy_load_threshold              = tc.value.auto_deploy_load_threshold
              auto_deploy_confidence_threshold        = tc.value.auto_deploy_confidence_threshold
              auto_deploy_impacted_baseline_threshold = tc.value.auto_deploy_impacted_baseline_threshold
              auto_deploy_expiration_sec              = tc.value.auto_deploy_expiration_sec
              detection_load_threshold                = tc.value.detection_load_threshold
              detection_absolute_qps                  = tc.value.detection_absolute_qps
              detection_relative_to_baseline_qps      = tc.value.detection_relative_to_baseline_qps
              dynamic "traffic_granularity_configs" {
                for_each = tc.value.traffic_granularity_configs
                iterator = tg
                content {
                  type                     = tg.value.type
                  value                    = tg.value.value
                  enable_each_unique_value = tg.value.enable_each_unique_value
                }
              }
            }
          }
  ...
  dynamic "advanced_options_config" {
    ...
    content {
      ...
      request_body_inspection_size = var.advanced_options_config.request_body_inspection_size
  ...
  dynamic "recaptcha_options_config" {
    for_each = var.recaptcha_options_config == null ? [] : [""]
    content {
      redirect_site_key = var.recaptcha_options_config.redirect_site_key
    }
  }
...
resource "google_compute_security_policy_rule" "global" {
  ...
  dynamic "match" {
    ...
    content {
      ...
      dynamic "expr_options" {
        for_each = (
          match.value.recaptcha_options == null
          ? []
          : [match.value.recaptcha_options]
        )
        iterator = ro
        content {
          recaptcha_options {
            action_token_site_keys  = ro.value.action_token_site_keys
            session_token_site_keys = ro.value.session_token_site_keys
          }
        }
      }
    }
  ...
  dynamic "header_action" {
    for_each = length(local.rules[each.key].header_action) == 0 ? [] : [""]
    content {
      dynamic "request_headers_to_adds" {
        for_each = local.rules[each.key].header_action
        content {
  ...
  dynamic "rate_limit_options" {
    ...
    content {
      ...
      dynamic "exceed_redirect_options" {
        for_each = (
          rl.value.exceed_redirect_options == null
          ? []
          : [rl.value.exceed_redirect_options]
        )
        content {
  ...
  dynamic "redirect_options" {
    for_each = (
      local.rules[each.key].redirect_options == null
      ? []
      : [local.rules[each.key].redirect_options]
    )
    content {
