...
resource "google_compute_subnetwork_iam_binding" "authoritative" {
  for_each   = var.subnet_create ? local.iam : {}
...
resource "google_compute_subnetwork_iam_binding" "bindings" {
  for_each   = var.subnet_create ? var.iam_bindings : {}
  project    = local.project_id
  region     = var.region
  subnetwork = var.name
  ...
  dynamic "condition" {
    ...
    content {
      expression = templatestring(
        each.value.condition.expression,
...
resource "google_compute_subnetwork_iam_member" "bindings" {
  for_each   = var.subnet_create ? var.iam_bindings_additive : {}
  project    = local.project_id
  region     = var.region
  subnetwork = var.name
  role       = lookup(local.ctx.custom_roles, each.value.role, each.value.role)
