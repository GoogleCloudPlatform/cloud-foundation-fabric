...
locals {
  cloud_config = templatefile(local.template, merge(local.config_variables, {
    frr_config           = templatefile(local.frr_config, local.config_variables)
    deamons              = templatefile(local.deamons, local.config_variables)
    files                = local.files
    enable_health_checks = var.enable_health_checks
    network_interfaces   = local.network_interfaces
  }))
  frr_config = (
    var.frr_config == null ? "${path.module}/frr.conf" : var.frr_config
  )
  deamons = (
    var.daemons == null ? "${path.module}/deamons" : var.daemons
  )
  files = merge({
