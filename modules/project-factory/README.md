...
- an optional `cicd_config` list which defines the CI/CD automation configurations for the project that configures Workload Identity Federation (WIF) and impersonation roles.
...
- an optional `templates` list which defines a set of templates for CI/CD workflows and provider files, which can be used to generate CI/CD pipelines or Terraform provider files. Note, this module does not generate any files.  See the [an example](../../fast/stages/2-project-factory/outputs-automation.tf)
