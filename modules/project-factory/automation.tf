...
locals {
  ...
  automation_outputs_buckets = {
    ...
    k => merge(try(v.automation.outputs_bucket, {}), {
      ...
      project_name       = v.name
      project_key        = k
    }) if try(v.automation.cicd_config, null) != null
  }
...
resource "google_service_account_iam_member" "sa_wif_binding" {
  ...
  service_account_id = module.automation-service-accounts["${each.value.project_key}/automation/${each.value.sa_key}"].id
  role               = "roles/iam.workloadIdentityUser"
  member = (
    each.value.branch == null
    ? format(var.federated_identity_providers[each.value.provider].principal_repo,
      var.federated_identity_pool,
    each.value.repository)
    : format(var.federated_identity_providers[each.value.provider].principal_branch,
      var.federated_identity_pool,
      each.value.repository,
    each.value.branch)
  )
