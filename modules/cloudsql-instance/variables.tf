...
variable "ssl_mode" {
  ...
  default     = null
  validation {
    condition     = var.ssl_mode == "ALLOW_UNENCRYPTED_AND_ENCRYPTED" || var.ssl_mode == "ENCRYPTED_ONLY" || var.ssl_mode == "TRUSTED_CLIENT_CERTIFICATE_REQUIRED"
...
variable "users" {
  ...
  type = map(object({
    ...
    ignore_password_change = optional(bool, false)
