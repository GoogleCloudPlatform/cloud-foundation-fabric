...
module "cloudsql" {
  ...
  data_api_access                   = try(each.value.data_api_access, null)
  ...
  managed_connection_pooling_config = try(each.value.managed_connection_pooling_config, {})
  network_config                    = try(each.value.network_config, { connectivity = {} })
  password_validation_policy        = try(each.value.password_validation_policy, null)
