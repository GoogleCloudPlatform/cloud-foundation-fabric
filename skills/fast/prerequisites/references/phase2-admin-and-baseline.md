...
1. Gather baseline information required for `0-org-setup`:
   ...
   *Action:* If the filtered `gcloud` command returns no results, inform the user and ask if they want to provide a different keyword or fetch all items (run without a filter).
   *Action:* Once you have results for Organizations (filtered or unfiltered), extract the Organization ID, the Display Name (domain), and the Directory Customer ID (found in `owner.directoryCustomerId` in the JSON output). Sort them alphabetically by the display name, and then present the results for the user to make their selection. Note the Organization ID, Domain, and Customer ID for Phase 4. For Billing Accounts, do the same sorting and selection.
