...
def fetch_module_readme(module_name):
    ...
    try:
        ...
        if e.code == 404:
            print(f"Module '{module_name}' not found or has no README.md.", file=sys.stderr)
        else:
...
def fetch_module_files(module_name, file_prefix):
    ...
    try:
        with urllib.request.urlopen(req) as response:
            data = json.loads(response.read().decode('utf-8'))
            for item in data:
                if item['type'] == 'file' and item['name'].startswith(file_prefix) and item['name'].endswith('.tf'):
                    file_name = item['name']
                    ...
                    try:
                        ...
                        print(f"Error fetching content for {file_name}: {e}", file=sys.stderr)
    except Exception as e:
        print(f"Error listing contents for {module_name}: {e}", file=sys.stderr)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch info about Cloud Foundation Fabric modules from GitHub.")
    parser.add_argument("module", nargs="?", help="Name of the module to fetch (e.g., 'project'). If omitted, lists all modules.")
    parser.add_argument("--latest-release", action="store_true", help="Fetch the latest release version.")
    parser.add_argument("--variables", action="store_true", help="Fetch variables files for the module.")
    parser.add_argument("--outputs", action="store_true", help="Fetch outputs files for the module.")
