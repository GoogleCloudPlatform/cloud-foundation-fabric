...
def main(dirs, exclude_file=None, files=False, show_diffs=False, show_extra=False):
    ...
    for dir_name in dirs:
        print(f"----- {dir_name} -----")
        for mod_name, state, diff in _check_dir(
            dir_name, exclude_file, files, show_extra
        ):
            if state == State.FAIL:
