...
FILE_RE_MODULES = re.compile(r'(?sm)module\s*"[^"]+"\s*\{[^\}]*?source\s*=\s*"([^"]+)"')
...
Variable = collections.namedtuple(
    "Variable", "name description type default required nullable source file line"
)
...
def format_files(items):
    ...
    yield "| name | description |{}{}".format(
        " modules |" if num_modules else "", " resources |" if num_resources else ""
    )
    yield "|---|---|{}{}".format(
        "---|" if num_modules else "", "---|" if num_resources else ""
    )
    for i in items:
        modules = resources = ""
        if i.modules:
...
def format_outputs(items, show_extra=True):
    ...
    if not items:
        return
    items.sort(key=lambda i: i.name)
    yield "| name | description | sensitive |" + (" consumers |" if show_extra else "")
    ...
    for i in items:
        ...
        for k in ("default", "type"):
            ...
            if "\n" in title:
                value = title.split("\n")
                # remove indent
                title = "\n".join([value[0]] + [v[2:] for v in value[1:]])
...
def create_doc(
    module_path, files=False, show_extra=False, exclude_files=None, readme=None
):
    if readme:
...
def main(
    module_path=None, exclude_file=None, files=False, replace=True, show_extra=True
):
    "Program entry point."
