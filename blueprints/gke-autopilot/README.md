...
<a href="../gke-autopilot/binauthz/" title="Binary Authorization Pipeline"><img src="../gke-autopilot/binauthz/diagram.png" align="left" width="280px"></a> This [blueprint](../gke-autopilot/binauthz/) shows how to create a CI and a CD pipeline in Cloud Build for the deployment of an application to a private GKE cluster with unrestricted access to a public endpoint. The blueprint enables a Binary Authorization policy in the project so only images that have been attested can be deployed to the cluster. The attestations are created using a cryptographic key pair that has been provisioned in KMS.
