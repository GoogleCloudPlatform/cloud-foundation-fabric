...
def compute_GKE_secondaryIP_utilization(config, read_mask, all_secondary_dict):
    ...
    for asset in response_cluster:
        ...
        cluster_service_rangeName = asset.resource.data['ipAllocationPolicy']['servicesSecondaryRangeName']
    ...
    for asset in response_node:
        ...
        for versioned in asset.versioned_resources:
            node_secondary_dict[node_full_name] ={
                ...
                'this_node_pool' : versioned.resource['metadata']['labels']['cloud.google.com/gke-nodepool'],
                'used_IP' : 0
            }
    ...
    for item in cluster_secondary_dict.values():
        ...
        all_secondary_dict[item['project']][itemKey]['used_ip_addresses'] += item['used_ip_addresses']
  ...
  for project_id in config["monitored_projects"]:
    ...
    for secondary_dict in secondaryRange_dict[project_id].values():
      ...
      metrics.append_data_to_series_buffer(
          config, metrics_dict["metrics_per_subnet"]["ip_usage_per_secondaryRange"]
          ["usage"]["name"], secondary_dict['used_ip_addresses'], metric_labels,
          timestamp=timestamp)
      metrics.append_data_to_series_buffer(
          config, metrics_dict["metrics_per_subnet"]["ip_usage_per_secondaryRange"]
          ["limit"]["name"], secondary_dict['total_ip_addresses'], metric_labels,
          timestamp=timestamp)
      metrics.append_data_to_series_buffer(
          config, metrics_dict["metrics_per_subnet"]["ip_usage_per_secondaryRange"]
