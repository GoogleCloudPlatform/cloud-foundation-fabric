...
from . import limits, metrics, networks, peerings, routers
...
def get_routes_for_router(config, project_id, router_region, router_name):
    ...
    """
    request = (
        config["clients"]["discovery_client"]
        .routers()
        .getRouterStatus(project=project_id, region=router_region, router=router_name)
    )
...
def get_routes_for_network(config, network_link, project_id, routers_dict):
    ...
    if network_link in routers_dict:
        for router_link in routers_dict[network_link]:
            ...
            routes = get_routes_for_router(
                config, project_id, router_region, router_name
            )
...
def get_dynamic_routes(config, metrics_dict, limits_dict):
    ...
    for project_id in config["monitored_projects"]:
        ...
        for network in network_dict:
            sum_routes = get_routes_for_network(
                config, network["self_link"], project_id, routers_dict
            )
            ...
            metrics.write_data_to_metric(
                ...
                project_id,
                sum_routes,
                metrics_dict["metrics_per_network"]["dynamic_routes_per_network"][
                    "usage"
                ]["name"],
