...
from . import limits, metrics, networks
...
def get_vpc_peering_data(config, metrics_dict, limit_dict):
    ...
    for project in config["monitored_projects"]:
        ...
        print(
            "Wrote number of active VPC peerings to custom metric for project:", project
        )
        ...
        for peering in vpc_peerings:
            metrics.write_data_to_metric(
                ...
                project,
                peering["peerings"],
                metrics_dict["metrics_per_network"]["vpc_peering_per_network"]["usage"][
                    "name"
                ],
                peering["network_name"],
            )
            metrics.write_data_to_metric(
                ...
                project,
                peering["network_limit"],
                metrics_dict["metrics_per_network"]["vpc_peering_per_network"]["limit"][
                    "name"
                ],
                peering["network_name"],
            )
            metrics.write_data_to_metric(
                config,
                project,
                peering["peerings"] / peering["network_limit"],
            ...
            if "peerings" in network:
                ...
                if STATE == "ACTIVE":
                    for peered_network in network[
                        ...
                        peered_net = {
                            "project_id": peered_project,
                            "network_name": peered_network_name,
                            "network_id": networks.get_network_id(
                                config, peered_project, peered_network_name
                            ),
                        }
