...
from . import limits, networks, peerings
...
def create_metric(metric_name, description, monitoring_project):
    ...
    descriptor = client.create_metric_descriptor(
        name=monitoring_project, metric_descriptor=descriptor
    )
    print("Created {}.".format(descriptor.name))


def write_data_to_metric(
    config, monitored_project_id, value, metric_name, network_name
):
    """
    ...
    try:
        client.create_time_series(
            name=config["monitoring_project_link"], time_series=[series]
        )
...
def get_pgg_data(config, metric_dict, usage_dict, limit_metric, limit_dict):
    ...
    for project in config["monitored_projects"]:
        ...
        for network_dict in network_dict_list:
            ...
            limit = networks.get_limit_network(
                network_dict, network_link, current_quota_limit_view, limit_dict
            )
            ...
            for peered_network_dict in network_dict["peerings"]:
                ...
                current_peered_quota_limit = limits.get_quota_current_limit(
                    config,
                    f"projects/{peered_network_dict['project_id']}",
                    limit_metric,
                )
                ...
                peered_limit = networks.get_limit_network(
                    peered_network_dict,
                    peered_network_link,
