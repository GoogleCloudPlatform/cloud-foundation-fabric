...
from metrics import ilb_fwrules, instances, metrics, networks, peerings, routes
    ...
    return monitoring_v3.TimeInterval(
        {
            ...
            "start_time": {"seconds": (seconds - 24 * 60 * 60), "nanos": nanos},
        }
    )
...
config = {
    ...
    "lb_scheme": {"L7": "INTERNAL_MANAGED", "L4": "INTERNAL"},
    "clients": {
...
def main(event, context):
    ...
    ilb_fwrules.get_forwarding_rules_data(
        config,
        metrics_dict,
        l7_forwarding_rules_dict,
        limits_dict["internal_forwarding_rules_l7_limit"],
        "L7",
    )
    peerings.get_vpc_peering_data(
        config, metrics_dict, limits_dict["number_of_vpc_peerings_limit"]
    )
    ...
    metrics.get_pgg_data(
        config,
        metrics_dict["metrics_per_peering_group"]["subnet_ranges_per_peering_group"],
        subnet_range_dict,
    ...
    routes.get_dynamic_routes_ppg(
        config,
        metrics_dict["metrics_per_peering_group"]["dynamic_routes_per_peering_group"],
        dynamic_routes_dict,
        limits_dict["number_of_subnet_IP_ranges_ppg_limit"],
    )
