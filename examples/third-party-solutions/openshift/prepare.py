...
def _run_installer(cmdline, env=None):
    ...
    try:
        p = subprocess.Popen(
            cmdline, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env or {}
        )
...
@click.option(
    "--tfdir", type=click.Path(exists=True), default="./tf", help="Terraform folder."
)
@click.option("--tfvars", help="Terraform vars file, relative to Terraform folder.")
@click.option(
    "-v",
    "--verbosity",
    default="INFO",
...
def cli(ctx=None, credentials=None, tfdir=None, tfvars=None, verbosity="INFO"):
    ...
    if ctx.invoked_subcommand is None:
        commands = ["install-config", "manifests", "manifests-edit", "ignition-configs"]
    else:
...
def manifests_edit(ctx=None):
    ...
    for fileobj in dir_.glob("99_openshift-cluster-api_worker-machineset-*.yaml"):
        ...
        try:
            ...
            if "ocp-worker" not in data_v["tags"]:
            ...
            data_n.insert(
                len(data_n), "projectID", tfvars["host_project"]["project_id"]
            )
    ...
    try:
        with fileobj.open() as f:
            ...
            config = [
                x
                for x in data["data"]["config"].strip().split("\n")
                if "network-" not in x.rpartition("=")[0]
            ]
            config += [
