...
One shape question stays open. `ca_pools` is a map of objects carrying `ca_ids`, `id` and `location`, and every other namespace is a flat map of strings. Either the security stage flattens it into separate namespaces on the way out, or context starts accepting structured values and the schema across producers gets weaker. This is the first case that tests whether a shared context schema is real, and it should be settled before anyone writes code.
