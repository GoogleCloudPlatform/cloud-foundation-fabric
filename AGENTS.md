...
Several modules support symbolic variable interpolation via a `context` variable. This allows callers to pass symbolic references like `"$project_ids:myprj"` instead of raw values, which get resolved at plan time. The [Context Reference](./CONTEXT-REFERENCE.md) doc explains the interface and its uses.
