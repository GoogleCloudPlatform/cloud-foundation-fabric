...
This configuration builds one chain in one VPC, but the attachment does not limit you to that, and the alternative matters wherever the environments are deliberately not connected to each other. A service attachment accepts concurrent connections from several consumers, so each VPC can build its own PSC NEG against the same attachment and front it with its own load balancer, its own address and its own DNS answer. Tested on 2026-09-14: a second NEG in a different VPC, in a different project, reached `ACCEPTED` with its own consumer address and its own connection id while the first stayed up.
