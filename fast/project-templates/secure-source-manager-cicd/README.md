...
That list is keyed on the project that owns the consumer resource, not on the host project of the network it connects from, and the difference decides what goes in it. Tested on 2026-09-14: a PSC NEG created in a project outside the list, on a Shared VPC network whose host project is inside it, sits at `pscConnectionStatus: PENDING` and never connects, while the same NEG in a listed project is `ACCEPTED` in seconds. So the list has to name every project that will ever create a NEG or an endpoint against the instance, which in a Shared VPC design means the service projects where load balancers live rather than the handful of host projects. Since it is immutable and allowing a project costs nothing, be generous at creation.
