...
**This path does not work yet.** Private Service Connect for private pools is gated: a well-formed `v1` create returns `400 INVALID_ARGUMENT: Private Service Connect feature is unavailable`, which is an allowlist on the project or organization. The field is documented and present in the `v1` API, so this is not a missing feature or a provider gap, and it is not related to second generation pools. Both Terraform and gcloud hide the failure — each omits the required `publicIpAddressDisabled`, so the API discards the whole PSC block and creates an ordinary pool with no network, successfully. See the Private Service Connect section in [SSM-CB.md](SSM-CB.md) for the full evidence and for what reverting to a peered network would cost. The rest of this section describes the intended design, for when the feature is available.
...
No external IP has no backstop, and that was tested rather than assumed. `no_external_ip` lives in `worker_config` and maps to the peered-network field `networkConfig.egressOption`; the API's `publicIpAddressDisabled` lives inside `privateServiceConnect` and the provider has no field for it. The `compute.vmExternalIpAccess` organization policy does not help, because the workers run in a Google-managed tenant project in Google's own organization, where our policies have no reach — a pool with `PUBLIC_EGRESS` was created in this project without complaint. So the requirement rests entirely on the pool's configuration being right, and nothing prevents a wrong one.
