...
**This is a work in progress.** The worker pool, the instance and the two load balancers are implemented and applied; the repositories and the build identity chain are not. The sections below record the design, the prerequisites, and the points still to be resolved.
...
The pool must allow CSR-based certificate requests, which is the default when no issuance policy is set, and the CA key must be 2048 bits. Either tier works: Secure Source Manager accepts a DevOps pool, which is what the security stage creates by default. The choice is a cost one. Enterprise is $200 per CA per month against $20 for DevOps, and what it buys is the ability to list, describe and revoke issued certificates. An instance holds one long-lived certificate, so the capability is rarely exercised and the difference is ten-fold; this playground uses DevOps, while a production landing zone that must be able to revoke should pay for Enterprise. The pool is immutable on the instance, so changing tier means recreating the instance, which takes about half an hour. The pool's project is independent of the instance's; its location is not. A pool in another region fails instance creation with a permission error that has nothing to do with permissions and leaves no trace in any log, so put the pool in the instance's region. Secure Source Manager runs in far fewer regions than Certificate Authority Service, and this is where that bites.
...
The auditor grant exists for one call. The instance's certificate is signed by our own CA, so nothing on a worker trusts it; before it can clone, a build runs `gcloud privateca pools get-ca-certs`, writes the chain and points git at it. A probe on 2026-09-14 confirmed both halves of that: a worker with no such grant is denied `privateca.caPools.get` and cannot obtain the chain, and a client that does hold the chain verifies the instance's certificate cleanly where a default trust store fails it. Auditor is read-only across the CA project's resources, which is wider than that one fetch, and it is granted per build identity on a project this configuration does not own. Two things would be better. A custom role holding only the permission `get-ca-certs` needs would keep the shape and narrow the grant. Publishing the bundle to a bucket, ideally a managed folder holding just this chain, would remove the grant and the delegation behind it entirely, since the chain is a public artifact and reading it needs nothing on the CA. Neither is done here; the second is on the TODO list as stage work.
...
The records point at the load balancer address, an ordinary internal address that peering, VPN and Interconnect all carry. One zone serves every connected network, and in a hub and spoke design it goes where the network-wide zones go: the hub, with the spokes reaching it through their peering zone. The address is an output of this configuration and the records are owned by the landing zone, so the zone is created first and the records filled in after the first apply. In the playground that is `pvt-ssm.yaml` under `2-networking/dns/zones/net-core-0`, and all four names take the same value.
...
A private instance publishes two Private Service Connect service attachments, one for HTTP and one for SSH, and both are computed rather than configured — the instance has no notion of a consumer and no way to create one. The only consumer-side control it carries is `private_config.psc_allowed_projects`, a list of projects permitted to connect, with the instance's own project allowed implicitly.

That list is keyed on the project that owns the consumer resource, not on the host project of the network it connects from. Tested on 2026-09-14: a PSC NEG created in a project outside the list, on a Shared VPC network whose host project is inside it, sits at `pscConnectionStatus: PENDING` and never connects, while the same NEG in a listed project is `ACCEPTED` in seconds.

So the list follows from where you put the load balancers, and that decision has to be made before the instance is created rather than after. Put each chain in its VPC's own project, which is the intended design here, and the list is the VPC projects. Put chains in service projects instead and it is those service projects, with the host projects contributing nothing. Since the list is immutable and naming a project costs nothing, be generous at creation and name anything that might plausibly front the instance later.

The failure mode deserves its own warning, because nothing reports it. Creating the NEG succeeds, Terraform applies clean, the load balancer builds on top of it without complaint, and the only symptom is that traffic never arrives. The status lives on the NEG, in `pscData.pscConnectionStatus`, and it is the first thing to read when a consumer chain is silent.
...
Two chains are forced, and then the address is what they share. A target TCP proxy references exactly one backend service, so each attachment needs its own NEG, backend service, target TCP proxy and forwarding rule; the alternative, SNI routing through a TLS route, needs the client to speak TLS and applies within one port, which excludes SSH. The address is the layer that does share: an internal address reserved with purpose `SHARED_LOADBALANCER_VIP` is the only kind several forwarding rules may reference, and the rules differ by port, so one address carries 443 to the HTTP chain and 22 to the SSH chain, and the zone holds a single A value for all four hostnames. Google's guide leaves the address to each forwarding rule and ends with two, which works and puts one more value in a landing zone that does not manage the address. Neither backend service sets a balancing mode: a PSC NEG backend accepts `UTILIZATION` and rejects `CONNECTION` and `RATE`, which is the opposite of what the documentation for regional internal proxy load balancers implies, and `UTILIZATION` is the module default.
...
The proxy carries no certificate, and cannot: these chains end in a target *TCP* proxy, which works at layer 4. It terminates the client's TCP connection and opens a separate one to the backend, and the TLS session rides inside that byte stream untouched — the resource has no `sslCertificates` field, because only target HTTPS and target SSL proxies have one. So the connection is terminated and the TLS session is not, and the handshake runs end to end from the client to the instance. Custom hostnames therefore do not require the load balancer; they require the CA pool, whose chain the client has to trust.

Choosing an HTTPS load balancer instead would move the certificate onto the proxy, put the load balancer inside the trust path, and require re-originating TLS to the instance. It would also not help with SSH. The same layer-4 property is why SNI cannot be used to put both services on one port: a TCP proxy never parses the TLS record, so it has nothing to route on.

### Several VPCs reaching one instance

This configuration builds one chain in one VPC, but the attachment does not limit you to that, and the alternative matters wherever the environments are deliberately not connected to each other. A service attachment accepts concurrent connections from several consumers, so each VPC can build its own PSC NEG against the same attachment and front it with its own load balancer, its own address and its own DNS answer.

Built and verified on 2026-09-14 rather than reasoned about. A second full chain — NEG, backend service, target TCP proxy, forwarding rule — was created in `ldj-prod-net-spoke-0`, a separate VPC, with the load balancer living in the VPC's own project as the design intends. Both connections held `ACCEPTED` at once with different connection ids and different consumer addresses, and both chains answered `401` to a verified HTTPS request for `git.ssm.gcp.qix.it`, each from its own address. The certificate returned through the second chain is the same instance certificate, signed by the same CA and carrying the same four names, so both TLS sessions terminate on the one instance and neither load balancer is in the trust path.

Nothing is shared between those consumers and nothing needs to be. There is no peering between the environments, no transitive routing to arrange, no shared VIP, and no peering group limits to count against — each environment's path is a private connection from its own VPC to the producer. The cost is one chain per environment to build and one more name to resolve per environment, and the constraint is the allowlist above: each chain's own project has to be named at instance creation, when the list is still writable. With a chain per VPC project, that is the VPC projects.

This template builds a single chain, in the instance project, which is the one project allowed implicitly. That is the test case rather than the pattern: it exercises the NEG, the backend service and the shared VIP against a real attachment without needing a second environment to exist.

A PSC NEG is the consumer construct here, and it targets the service attachment directly — `pscTargetService` holds the attachment URI, and the NEG itself carries the connection id, the consumer address and the accept status. A PSC endpoint, a forwarding rule whose target is the attachment, is the alternative to a NEG rather than something a NEG is built on; this design uses NEGs because they can sit behind a load balancer, which is what makes the address reachable beyond its own VPC.

All of this was probed on 2026-09-14 rather than left as an argument, from a build worker on the peering and from a bastion in the hub. Both resolve the four hostnames to the one address and open both ports on it, across a region boundary and a peering in each case, and the instance answers. The handshake returns the instance's own certificate, signed by our CA and carrying the four custom names in its subject alternative name list, which is what confirms the TLS session reaches the instance. [SSM-CB.md](SSM-CB.md) has the results; `builds/probe-ssm.yaml` is the build.
...
- **What the load balancers need in a Shared VPC service project.** Answered by building it: `roles/compute.networkUser` at host project level for the automation account and for the compute service agent, which is where that role belongs. Subnet-scoped grants are not the narrower form of this — IAM composes, so a subnet binding adds to a project binding rather than restricting it, and limiting a service project to particular subnets is an organisation policy question rather than an IAM one.
