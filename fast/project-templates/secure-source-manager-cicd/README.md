...
Attach the zone only to networks that hold an endpoint. In a hub and spoke design the hub usually holds the network-wide zones and the spokes reach them through a peering zone, but an endpoint address is the opposite of network-wide: it is valid only inside the VPC that holds it, and a second network gets a different address for the same name. So with endpoints the zone is per network, attached to the spoke that holds the endpoint and shadowing the peering zone for its own suffix, and a second network that gets its own endpoint gets its own zone for the same domain. Attaching a spoke's zone to the hub as well, which is a common pattern for other spoke zones, makes a hub client resolve the name to an address it cannot reach and fail with a connect timeout rather than a name error. The zone moves to the hub, as a single network-wide record, only with the load balancer fallback below, where one address serves every connected network. That is a second reason to prefer the fallback once more than one network needs the instance: it is the design that matches a centralised DNS layout, not only the one that reaches on-premises.
