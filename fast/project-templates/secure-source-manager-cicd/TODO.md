...
- [ ] **`context` in `modules/secure-source-manager-instance`, now load-bearing.** With logical names in the template, `$ca_pools:dev-ca-0` has to resolve somewhere. Either the template resolves it before passing it down, or the module gets a `context` variable. This is the second time it has come up.
