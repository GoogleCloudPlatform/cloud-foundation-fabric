...
- **PSC NEG backends reject a balancing mode** (2026-09-13). Found while applying this template's load balancers. Six modules in the load balancer family create PSC NEGs and every one of them sends `balancing_mode` unconditionally, declared `optional(string, <default>)` so a caller cannot omit it; upstream master is identical, and no issue or pull request in the repository records it. Only `net-lb-proxy-int` recognises a PSC backend, and only to suppress the health check; `net-lb-proxy-int-cross-region` looks like it carries the same bug twice over, health check included. Nothing caught it because the family's two PSC fixtures are plan tests and a plan never asks the API. Evidence, per-class probes and fix shapes are in `~/dev/tf-playground/LB-PSC-WORK.md`.
