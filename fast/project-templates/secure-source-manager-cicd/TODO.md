...
- [ ] **The two load balancers — blocked on an upstream module bug.** `ssm-load-balancers.tf` is written and plans clean: one reserved `SHARED_LOADBALANCER_VIP` address and two `net-lb-proxy-int` chains, 443 to the HTTP attachment and 22 to the SSH one. The apply creates the address `test-0-dev-0-ssm-lb` and both PSC NEGs, then fails on the backend services, because the API refuses any balancing mode on a PSC NEG backend and the module always sends one. Those three resources are live and in state; the backend services are not. Everything gathered about it — which modules are affected, the probes that scope it per LB class, the live NEGs to probe against, the fix shapes — is in `~/dev/tf-playground/LB-PSC-WORK.md`, with the fix on branch `ludo/lb-psc-fixes` off master. Carry the one-line module change here until it lands upstream, then this item is a re-run of the apply.
