...
The infrastructure we bring up exists to answer these, and the first two can each invalidate the design above. Neither would require rebuilding the instance, since both are about trigger behaviour rather than instance configuration, so run them as soon as the instance is up and before anything else is built:
