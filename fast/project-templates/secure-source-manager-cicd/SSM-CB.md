...
5. Find out what a worker pool with no external IP and `route_all_traffic` can reach, which with a network attachment is a question about your own Cloud NAT and egress rules rather than about the pool. If we revisit Developer Connect, establish whether the proxy at `REGION-git.developerconnect.dev` is reachable through private.googleapis.com, and price what adding that VIP does to the perimeter.
   ...
   So test it as attribution rather than as routing, which we cannot read: make a build step call an API the perimeter denies, and confirm the violation appears in our perimeter's audit logs with the worker as the source. Denied somewhere we cannot see, or not denied at all, means the routing assumption is wrong. The external IP half needs no test of its own — `compute.vmExternalIpAccess` reaches the tenant instances, so a pool that would get public addresses fails creation.
