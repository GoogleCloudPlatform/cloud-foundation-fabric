...
The tier was a red herring. A DevOps pool in the instance's region, `dev-ca-3`, created a working instance on 2026-09-12: the throwaway `tiertest-0` reached completion in 32 minutes 45 seconds before being deleted. Enterprise is not a requirement and the security stage's `create_pool: {}` default is usable. The one thing that test did not cover is `custom_host_config`, which it was created without.
...
The certificate is the part worth keeping. The TLS handshake on 443 presents `O = Google, CN = sourcemanager.dev`, issued by `O = Test Example, CN = test.example.com`, which is `dev-ca-3`, with a subject alternative name list holding exactly the four custom hostnames. So TLS runs from the client to the instance and the CA pool's only job in the data path is having signed those names.

The reason is the kind of proxy, and it is worth stating precisely because "proxy" covers two different terminations. `net-lb-proxy-int` builds a target *TCP* proxy, which is layer 4: it terminates the client's TCP connection and opens a separate one to the backend, splicing bytes between them, and the TLS session travels inside those bytes untouched. The resource has no `sslCertificates` field — only target HTTPS and target SSL proxies have one — and the project holds no certificate of any kind. So the connection is terminated and the TLS session is not, which is why the handshake reaches the instance and returns the instance's certificate.

Two consequences follow. A target HTTPS proxy would need its own certificate, would terminate TLS, and would have to re-originate it to the instance, which would make the instance's CA-signed custom hostnames half-redundant and put the load balancer inside the trust path. And a TCP proxy cannot route on SNI, because it never parses the TLS record, which is the other half of why SNI-based sharing of one port was dropped in favour of two chains. Verification fails with code 21 against a default trust store and succeeds against the chain from `gcloud privateca pools get-ca-certs`, which is the caveat about trust stores made concrete rather than a new finding.
...
The consequence is that the list follows from where the chains live, and because it is immutable, that placement has to be decided before the instance is created. The intended design puts each chain in its own VPC's project, so the list is the VPC projects and reads the same either way — which is why the error was invisible until probed. It stops reading the same the moment a chain lives anywhere else: a chain in a service project needs that service project named, and its host project being named contributes nothing. Naming host projects and expecting the service projects on them to inherit is the mistake, and it cannot be corrected without deleting the instance.
...
8. Closed on 2026-09-12: creation took 29 minutes 28 seconds, from the create operation's own `createTime` to its `endTime`, and a second instance the same day took 32 minutes 45 seconds. The documentation's "up to 60 minutes" is honest and the provider's 120 minute timeout is ample. Read the duration from `gcloud source-manager operations list`, not from the instance: `update_time` on the resource keeps moving long after creation and is not a completion time.
   ...
   The same instance settled three smaller things. Custom hostnames replace the generated ones outright: `host_config` comes back identical to `custom_host_config`, with no `p.sourcemanager.dev` name anywhere in the resource, so there is only one set of names to resolve. The service attachments are `projects/<tenant>-tp/regions/europe-west4/serviceAttachments/http-psc` and `.../ssh-psc`, and the tenant project is different on every rebuild, so no configuration may hardcode them. Instance ids, unlike CA pool ids, are reusable after deletion: `test-0-dev-0` was destroyed and recreated under the same name the same afternoon.
