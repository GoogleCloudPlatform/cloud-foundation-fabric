  ...
  network_interfaces = [
    {
      ...
      subnetwork = try(
        module.dmz-vpc.subnet_self_links["${each.value.region}/dmz-default-${each.value.shortname}"], null
      )
      nat       = false
      addresses = null
    },
    {
      ...
      subnetwork = try(
        module.landing-vpc.subnet_self_links["${each.value.region}/landing-default-${each.value.shortname}"], null
      )
      nat       = false
      addresses = null
    }
