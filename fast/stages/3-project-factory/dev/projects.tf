...
locals {
  ...
  _projects_raw = {
    for f in local.project_files : trimsuffix(f, ".yaml") => yamldecode(
      file("${coalesce(var.factories_config.hierarchy_path, "-")}/${f}")
    )
  }
  projects = {
...
module "projects" {
  ...
  parent = lookup(
    local.hierarchy_parents, each.value.parent, each.value.parent
  )
  prefix                    = var.prefix
