...
locals {
  ...
  _of_providers_single = {
    for k, v in local._of.providers : k => merge(v, {
      filename = k
      service_account = try(
        module.factory.projects[k].automation.service_accounts["${k}/${v.service_account}"].email,
        null
      )
    })
    if lookup(module.factory.service_account_emails, "${k}/${v.service_account}", null) != null
  }
...
resource "local_file" "providers" {
  ...
  filename        = "${local.of_path}/providers/${local.of_prefix}/${each.value.filename}.tf"
