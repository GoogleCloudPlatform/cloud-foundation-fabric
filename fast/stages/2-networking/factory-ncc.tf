...
locals {
  ...
  ctx_ncc_hubs = { for k, v in google_network_connectivity_hub.default : k => v.id }
...
resource "google_network_connectivity_group" "default" {
  ...
  hub         = lookup(local.ctx_ncc_hubs, replace(each.value.hub, "$ncc_hubs:", ""), each.value.hub)
  labels      = each.value.labels
  description = each.value.description
  dynamic "auto_accept" {
    for_each = try(each.value.auto_accept != null, false) ? [""] : []
    content {
      auto_accept_projects = [for project_key in try(each.value.auto_accept, []) : lookup(local.ctx_projects.project_ids, replace(project_key, "$project_ids:", ""), project_key)]
    }
  }
...
resource "google_network_connectivity_spoke" "vpcs" {
  ...
  hub         = lookup(local.ctx_ncc_hubs, replace(each.value.hub, "$ncc_hubs:", ""), each.value.hub)
  linked_vpc_network {
    uri                   = lookup(local.ctx_vpcs.self_links, each.value.vpc_self_link, each.value.vpc_self_link)
...
resource "google_network_connectivity_spoke" "tunnels" {
  ...
  project     = lookup(local.ctx_projects.project_ids, replace(each.value.project_id, "$project_ids:", ""), each.value.project_id)
  name        = replace(each.key, "/", "-")
  location    = lookup(local.ctx.locations, replace(each.value.location, "$locations:", ""), each.value.location)
  description = each.value.description
  labels      = each.value.labels
  hub         = lookup(local.ctx_ncc_hubs, replace(each.value.hub, "$ncc_hubs:", ""), each.value.hub)
  linked_vpn_tunnels {
