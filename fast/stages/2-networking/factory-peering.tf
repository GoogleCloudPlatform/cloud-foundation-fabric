...
locals {
  ...
  peering_configs = merge(flatten([
    for vpc_key, vpc_config in local.vpcs : {
      for peering_key, peering_configs in try(vpc_config.peering_config, {}) : "${vpc_key}/${peering_key}" => merge(peering_configs, {
...
resource "google_compute_network_peering" "default" {
  ...
  network                             = lookup(local.ctx_vpcs.self_links, replace(each.value.local_network, "$vpcs:", ""), each.value.local_network)
  peer_network                        = lookup(local.ctx_vpcs.self_links, replace(each.value.peer_network, "$vpcs:", ""), each.value.peer_network)
