...
locals {
  ...
  nva_configs = {
    ...
    join("-", v) => {
      ...
      ip_neighbor_cross_region_nva_0 = cidrhost(module.landing-untrusted-vpc.subnet_ips[local.landing_subnet_ids[local._regions_cross[v.0]].untrusted], 101)
      ip_neighbor_cross_region_nva_1 = cidrhost(module.landing-untrusted-vpc.subnet_ips[local.landing_subnet_ids[local._regions_cross[v.0]].untrusted], 102)
      # The Cloud router IPs (x.y.w.z) in the untrusted
      # and in the trusted VPCs, where the NVA connects to
      ip_neighbor_trusted_0   = cidrhost(module.landing-trusted-vpc.subnet_ips[local.landing_subnet_ids[v.0].trusted], 201)
      ip_neighbor_trusted_1   = cidrhost(module.landing-trusted-vpc.subnet_ips[local.landing_subnet_ids[v.0].trusted], 202)
      ip_neighbor_untrusted_0 = cidrhost(module.landing-untrusted-vpc.subnet_ips[local.landing_subnet_ids[v.0].untrusted], 201)
      ip_neighbor_untrusted_1 = cidrhost(module.landing-untrusted-vpc.subnet_ips[local.landing_subnet_ids[v.0].untrusted], 202)
      # The IPs to assign to the NVA NICs
      # in the trusted and in the untrusted VPCs.
      ip_trusted   = cidrhost(module.landing-trusted-vpc.subnet_ips[local.landing_subnet_ids[v.0].trusted], 101 + index(var.zones, v.1))
      ip_untrusted = cidrhost(module.landing-untrusted-vpc.subnet_ips[local.landing_subnet_ids[v.0].untrusted], 101 + index(var.zones, v.1))
      # subnetwork ids
      subnetwork_name_untrusted = local.landing_subnet_ids[v.0].untrusted
      subnetwork_name_trusted   = local.landing_subnet_ids[v.0].trusted
...
resource "google_compute_address" "nva_static_ip_trusted" {
  ...
  subnetwork   = module.landing-trusted-vpc.subnet_self_links[each.value.subnetwork_name_trusted]
...
resource "google_compute_address" "nva_static_ip_untrusted" {
  ...
  project      = module.landing-project.project_id
  subnetwork   = module.landing-untrusted-vpc.subnet_self_links[each.value.subnetwork_name_untrusted]
  address_type = "INTERNAL"
