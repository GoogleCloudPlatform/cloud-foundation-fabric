...
variable "tenant_configs" {
  description = "Tenant configurations. Keys are used for short names and must be 4 characters or less."
  type = map(object({
  ...
  validation {
    condition = alltrue(flatten([
      for k, v in var.tenant_configs : [
        for a in ["descriptive_name", "groups"] : v[a] != null
      ]
    ]))
  ...
  validation {
    condition = alltrue([
      for k, v in var.tenant_configs : length(k) < 5
    ])
