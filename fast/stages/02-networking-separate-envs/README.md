...
This architecture creates one VPC for each environment, each in its respective project. Thos VPCs host external connectivity and shared services solely serving its own environment. VPCs are configured as a Shared VPC host, so that network-related resources and access configurations via IAM are kept separate.
