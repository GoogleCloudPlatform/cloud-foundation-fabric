                ...
                for repository, name in self.utils.get_repositories().items():
                    ...
                    if stage:
                        ...
                        for path in Path(temp_dir).rglob('*.tf'):
                            ...
                            self.utils.change_module_source(
                                f"{temp_dir}/{path_name}",
                                module_repository,
                                tag=module_tag)
                    ...
                    if git_origin:
                        ...
                        git.run(["remote", "add", "origin", git_origin],
                                temp_dir)

                        print(f"Fetching origin")
                        fetch_ok, fetch_stderr = git.run(
                            ["fetch", "origin", "main"],
                            temp_dir,
                            allow_fail=True)
                        ...
                        git.run(["commit", "-a", "-m", "Fabric FAST import"],
                                temp_dir)
...
class FastCicdSystemConfigurator(FastDialogs):
    ...
    def select_final_config(self):
        ...
        if choice:
            ...
            self.utils.write_bootstrap_config(
                self.config["bootstrap_output_path"], self.config)
            return True
        return False
