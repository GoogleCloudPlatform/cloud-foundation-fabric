...
This stage sets up VPC Service Controls (VPC-SC) for the whole organization via a thin wrapper on the [VPC-SC module](../../modules/vpc-sc/).
...
- [Design overview and choices](#design-overview-and-choices)
  ...
  - [Context replacement support](#context-replacement-support)
...
This stage exposes the underlying vpc-sc module support for context replacement, which allows writing portable factory files which interpolate actual values (project numbers, identities, etc.) at run time.

The `context` variable supports four different replacement types, some of which are pre-populated with values coming from the previous stage:

- `identity_sets`, pre-populated with sink writer identities at the organization level and can be used via `$identity_sets:logging_identities`
- `project_numbers`, pre-populated with numbers for projects created in the previus stage
- `resource_sets`, pre-populated with discovered projects if the discovery feature is enables, usable via `$resource_sets:discovered_projects`
- `service_sets`, pre-populated with values from the YAML services file, usable via `$service_sets:restricted_services`

Pre-populated contexts are merged with contexts passed in via the `context` variable. The default factory files with this stage show some usage examples of context replacements.
...
| [logging](variables-fast.tf#L26) | Log writer identities for organization / folders. | <code title="object&#40;&#123;&#10;  writer_identities &#61; map&#40;string&#41;&#10;&#125;&#41;">object&#40;&#123;&#8230;&#125;&#41;</code> |  | <code>null</code> | <code>0-bootstrap</code> |
...
| [project_numbers](variables-fast.tf#L46) | Project numbers. | <code>map&#40;number&#41;</code> |  | <code>&#123;&#125;</code> | <code>0-bootstrap</code> |
...
| [root_node](variables-fast.tf#L54) | Root node for the hierarchy, if running in tenant mode. | <code>string</code> |  | <code>null</code> | <code>0-bootstrap</code> |
